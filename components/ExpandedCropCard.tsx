...
const ExpandedCropCard: React.FC<ExpandedCropCardProps> = ({ crop, language, onClose }) => {
  useEffect(() => {
    // NOTE: We no longer add/remove 'overflow-hidden' to the body.
    // This allows the modal itself to be scrollable if its content exceeds the viewport height.
  ...
  return (
    <div 
      className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-sm animate-backdropFadeIn"
      onClick={onClose}
      aria-modal="true"
      role="dialog"
    >
      ...
      >
        <div 
          className="bg-white dark:bg-brand-dark-secondary rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col md:flex-row animate-slideInDown relative border border-gray-200 dark:border-gray-700"
        ...
        >
          ...
          <img src={crop.imageUrl} alt={crop.cropName} className="w-full md:w-1/2 h-64 md:h-auto object-cover rounded-t-2xl md:rounded-l-2xl md:rounded-t-none flex-shrink-0" />
